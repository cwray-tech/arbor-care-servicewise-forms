<script type="text/javascript">(function(){ var sa = {
    form: {
        rootEl: null,
        init: []
    },
    utils: {
        ...
        {
            if (sa.form.rootEl == null) { return; }
            if (scrollToTop)
            {
                let target = window.parent.document.querySelector('#formDialog')?.parentElement ||
                    window.parent.document.querySelector('#viewOverlayBodyRegionContainer');
...
document.addEventListener("DOMContentLoaded", InitializeForm);var SaValidation =
{
    ...
    email: function (field, isValid, isRequired)
    {
        ...
        if ((input.value === "" && isRequired) || (input.value !== "" && !/(.+)@(.+){2,}\.(.+){2,}/.test(input.value))
    ...
    ccupdate: function (field, isValid, isRequired)
    {
        ...
        if (isRequired)
        {
            ...
            if (isValid.value)
                button.classList.remove('sa-invalid');
            else
                button.classList.add('sa-invalid');
            return;
        }
...
var SaValidationStyle =
{
    number: function (field)
    {
        ...
        input.addEventListener("change", function (event)
        {
            if (event.target.checkValidity())
                ...
                event.target.classList.add("sa-invalid");
        });
    },
        ...
        for (let j = 0; j < checkboxes.length; j++)
        {
            checkboxes[j].addEventListener("change", function (event)
            {
                if (event.target.checked)
                {
                    ...
                    if (wrapper)
                        wrapper.classList.remove("sa-invalid-label");
                }
            });
        }
    },
        ...
        for (let j = 0; j < ratingInputs.length; j++)
        {
            ratingInputs[j].addEventListener("change", function (event)
            {
                if (event.target.checked)
                {
                    ...
                    if (wrapper)
                        wrapper.classList.remove("sa-invalid-rating");
                }
            });
        }
    },
    ...
    {
        const config = { attributes: true, childList: false, subtree: false, attributeFilter: ['data-ccresult'] };
...
function ValidateFields(page)
{
    ...
    for (let i = 0; i < fields.length; i++)
    {
        ...
        if (SaValidation[fieldType] == null)
            continue;
...
function UpdateInputValidStyle()
{
    ...
    for (let i = 0; i < fields.length; i++)
    {
        ...
        if (fieldType !== "attachment")
            SaValidationStyle[fieldType](field);
    }
}
...
sa.form.init.push(UpdateInputValidStyle);function LoadSubmitButton(confirmationType, data)
{
...
function SubmitForm(page, success) {
    ...
    const data = { request: { formTemplateId: '3323eec2-4045-43a7-8883-d57ea6fbe13b', submittedById: urlVars["sId"], submittedByType: urlVars["sType"], parentId: urlVars["pId"], parentType: urlVars["pType"], source: urlVars["source"], data: submitData } };
...
sa.form.init.push(InitializeCharacterCounters);var saFormUploads = saFormUploads || { Pending: [] };
...
function PageModel(pageIndex, pageElement)
{
    ...
    self.GetFieldValues = function ()
    {
        ...
        if (window.saUploadFiles === null)
            window.saUploadFiles = [];
        ...
        for (let i = 0; i < inputs.length; i++)
        {
            ...
            {
                case "longText":
                    self.fields.push({ fieldId: fieldId, value: input.querySelector("textarea").value });
                    break;                
                case "dropdown":
                    self.fields.push({ fieldId: fieldId, value: input.querySelector("select").value });
                    break;
                case "radio":
                case "rating":
                    {
                ...
                case "review":
                    {
                        ...
                        self.fields.push({ fieldId: fieldId, values: [reviewValue, input.querySelector("textarea").value ] });
                    }
                    break;
                case "attachment":
                    {
                        if (saFormUploads === undefined || saFormUploads === null || saFormUploads.Uploads === undefined ||  saFormUploads.Uploads === null || saFormUploads.Uploads.length === 0)
                            break;
                ...
                case "hidden":
                    ...
                    break;
                default:
                    self.fields.push({ fieldId: fieldId, value: input.querySelector("input").value });
                    break;
            }
        }
    };
...
sa.form.init.push(InitializePaging); } )(); //# sourceURL=SaForms.js</script>